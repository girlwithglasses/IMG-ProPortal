...
<input type='button' class='smdefbutton'
 ...
 >
    ...
    my @tabIndex = (
        "#biodetailtab1", "#biodetailtab2", "#biodetailtab3", "#biodetailtab4",
        "#biodetailtab5", "#biodetailtab6", "#biodetailtab7", "#biodetailtab8"
    );
    ...
    my ( $taxon_display_name, $gold_id, $scaffold_oid, $scaffold_name ) = $cur->fetchrow();
    ...
    if ( $in_file eq 'Yes' ) {
        $s_url = "$main_cgi?section=MetaScaffoldDetail"
    ...
    GeneDetail::printAttrRowRaw( "DNA Sequence", alink( $seq_url, "$min..$max" ) );
    ...
    my $gene_count = getGeneCountForCluster( $dbh, $taxon_oid, $in_file, $cluster_id );
    ...
    print "<div id='biodetailtab8'>";
    printPairwiseTab( $cluster_id, $taxon_oid );
    print "</div>";    # end biodetailtab8
...
sub printFastaForCluster {
    ...
    my $seq;
    if ( $in_file eq "Yes" ) {
        my $allseq = MetaUtil::getScaffoldFna( $taxon_oid, "assembled", $scaffold_oid );
        ...
        $seq = WebUtil::readLinearFasta( $path, $ext_accession, $start, $end, "+" );
    }
...
sub exportClusterGenbankFile {
    ...
    print "<h1>Export Biosynthetic Cluster Genbank File</h1>\n";
    print "<p style='width: 650px;'>";
    my $url1 = "$main_cgi?section=TaxonDetail&page=taxonDetail&taxon_oid=$taxon_oid";
    print "Genome: " . alink( $url1, $taxon_name );
    my $url2 = "$section_cgi&page=cluster_detail&cluster_id=$cluster_id";
...
sub printBioClusterGeneList {
    ...
    if ( $isTaxonInFile || $genome_type eq 'metagenome' ) {
        ...
        getBcBestHit_phylodist( $dbh, $taxon_oid, $cluster_id, \%bbh_gene, \%bbh_taxon, \%taxon_name_h );
    } else {
        if ( WebUtil::isInt($cluster_id) ) {
            getBcBestHit_bbh( $dbh, $taxon_oid, $cluster_id, \%bbh_gene, \%bbh_taxon, \%taxon_name_h );
        }
        else {
    ...
    my ($evid_type);
    if ( WebUtil::isInt($cluster_id) ) {
        my $sql      = "select evidence from bio_cluster_data_new " . "where cluster_id = ? ";
        my $cur      = execSql( $dbh, $sql, $verbose, $cluster_id );
        ($evid_type) = $cur->fetchrow();
        $cur->finish();
    }
    else {
        ($evid_type) = 'ClusterScout';
    }
    ...
    if ( $isTaxonInFile ) {
        ...
        if ( WebUtil::isInt($cluster_id) ) {
            my $sql = qq{
                ...
                from bio_cluster_features_new bcg, pfam_family pf, gene_pfam_families gpf
                where bcg.cluster_id = ?
                and bcg.feature_type = 'gene'
            ...
            my $sql = qq{
                select distinct gpf.gene_oid, 1, 1, gpf.pfam_family, pf.description
                from scaffold s, gene g, pfam_family pf, gene_pfam_families gpf
    ...
    for ( ;; ) {
        ...
        my $str = "";
        if ($isTaxonInFile) {
            my @pfamids = MetaUtil::getGenePfamId( $gene_oid, $taxon_oid, 'assembled' );
        ...
        if ( $isTaxonInFile || $genome_type eq 'metagenome' ) {
            ...
            if ($bbh) {
                ...
                if ( !$taxon_name_h{$homo_taxon} ) {
                    $taxon_name_h{$homo_taxon} = WebUtil::taxonOid2Name( $dbh, $homo_taxon );
                }
            }
        }
...
sub printMyIMGBioClusterNPList {
    ...
    for ( ;; ) {
        my (
            ...
            $comments,     $modified_by_oid, $modified_by,  $img_group,     $mod_date
          )
          = $cur->fetchrow();
        last if ( !$clu_id );
        ...
        if ( $contact_oid == $modified_by_oid ) {
            ...
            # my annotation
            $r .= $sd
              . "<input type='radio' id='myimg_bc_np' "
              . "name='myimg_bc_np' "
...
sub getBcBestHit_phylodist {
    my ( $dbh, $taxon_oid, $cluster_id, $bbh_gene_h, $bbh_taxon_h, $taxon_name_h ) = @_;
...
sub printBioClusterPfamList {
    ...
    my $url2;
    if ( WebUtil::isInt($cluster_id) ) {
        $url2 = "$section_cgi&page=cluster_detail&cluster_id=$cluster_id";
    }
    else {
        my ( $scaffold_oid, $start_coord, $end_coord ) = split( / /, $cluster_id );
    ...
    if ( $in_file eq "Yes" ) {
        ...
        foreach my $pfam_id (@pfamids) {
            my $r;
            $r .= $sd . "<input type='checkbox' name='func_id' value='$pfam_id' /> \t";
...
sub printBCPfamGeneList {
    ...
    my $dbh = dbLogin();
    my ( $taxon_oid, $isTaxonInFile, $cluster_id, $pfam_ids_ref,
    ...
    print "<br/>Cluster ID: " . alink( $url2, $cluster_id );
    print "<br/>Pfam: ";
    print "<br/>" if (scalar(@$pfam_ids_ref) > 1);
...
sub getBCPfamGeneList {
    ...
    if ($isTaxonInFile) {
        ...
        my $funcIdsInClause = TaxonDetailUtil::fetchPfamId2NameHash( $dbh, \@pfam_ids, \%funcId2Name, 0 );
      ...
      GENE: for ( ;; ) {
        ...
        if ( WebUtil::isInt($cluster_id) ) {
            ...
            my $sql = qq{
                select distinct gpf.gene_oid
                from scaffold s, gene g, gene_pfam_families gpf
                where s.scaffold_oid = ?
                and g.scaffold = s.scaffold_oid
                and ((g.end_coord > ? and g.end_coord <= ?) or (g.start_coord >= ? and g.start_coord < ?))
                and g.start_coord > 0
    ...
    return ( $taxon_oid, $isTaxonInFile, $cluster_id, \@pfam_ids, \%funcId2Name, \@gene_oids );
}
...
sub printNeighborhoods {
    ...
    my $fnurl =
        "xml.cgi?section=$section&page=reload_viewer"
      . "&taxon_oid=$taxon_oid&cluster_id=$cluster_id"
...
sub loadNeighborhoods {
    ...
    foreach my $r (@$recs_ref) {
        my ( $tx, $gene_oid, $panelStrand, $q_gene, $hitnum ) = split( /\t/, $r );
        ...
        if ( $taxon_in_file_h{$tx} ) {
            ...
            my $sql2 = "select taxon_display_name, in_file " . "from taxon where taxon_oid = ?";
        ...
        if ( $s2_in_file eq 'No' && isInt($gene_oid) ) {
            ( $scaffold_oid, $scaffold_name, $topology, $scf_length, $start_coord, $end_coord, $strand ) =
            ...
            if ( scalar(@vals) >= 7 ) {
                ...
                if ($scaffold_oid) {
                    my ( $s2, $e2, $r2 ) = MetaUtil::getScaffoldCoord( $tx, 'assembled', $scaffold_oid );
                    $scf_length = $e2 - $s2 + 1;
                }
            }
        }
        ...
        $scaffold_info{$scaffold_oid} = $scaffold_name . "\t" . $tx . "\t" . $topology . "\t" . $scf_length;
...
sub printNeighborhoodPanels {
    ...
    foreach my $scf ( keys %$cluster_href ) {
        my ( $scaffold_name, $tx, $topology, $scf_length ) = split( /\t/, $scf_info_href->{$scf} );
        ...
        my $clurl;
        if ( WebUtil::isInt($cluster_id) ) {
            $clurl = "$section_cgi&page=cluster_detail&cluster_id=$cluster_id";
        }
        else {
            my ( $scaffold_oid, $start_coord, $end_coord ) = split( / /, $cluster_id );
...
sub printOneNeighborhood {
    ...
    if ( !$infile ) {
        ...
        my @genes_on_s = MetaUtil::getScaffoldGenes( $taxon_oid, "assembled", $scaffold_oid );
    ...
    my $scf_url = "$main_cgi?section=ScaffoldDetail&page=scaffoldDetail"
	        . "&scaffold_oid=$scaffold_oid&taxon_oid=$taxon_oid";
    $scf_url = "$main_cgi?section=MetaScaffoldDetail"
	     . "&page=metaScaffoldDetail&taxon_oid=$taxon_oid"
    ...
    if ($bbhlite) {
		...
		= split( /\t/, $r );
            ...
            if ( $staxon && $validTaxons{$staxon} ) {
                ...
                my $rec = "$staxon\t$sgene_oid\t$panelStrand\t$qgene_oid\t$numhits";
                push( @recs, $rec );
            }
        }
    ...
    if ( $mygene ne "" ) {
        ...
        if ( !$gid || $gid ne $mygene ) {
            ...
            if ( $in_file eq "Yes" ) {
                ...
                MetaGeneDetail::printGeneDetail("$taxon_oid assembled $mygene");
            } else {
                require GeneDetail;
        ...
        foreach my $sim_bc (@$sim_aref) {
            ...
            for ( ;; ) {
                ...
                if ( $staxon && $public_taxons{$staxon} ) {
                    ...
                    my $panelStrand = $strand1 eq $strand2 ? "+" : "-";
                    my $rec = "$staxon\t$sgene_oid\t$panelStrand\t$gene_oid\t$numhits";
                    push( @recs, $rec );
                }
            }
            $cur2->finish();
...
sub printClusterPathwayEvidence {
    ...
    for ( my $i = 0 ; $i < $nReactions ; $i++ ) {
        ...
        my ( $rxn_oid_prev, $rxn_order_prev, $rxn_name, $rxn_definition, $is_mandatory ) = split( /\t/, $r_prev );
        my $subOrderPrev = $rxnOid2SubOrder{$rxn_oid_prev};

        my $r_curr = $reactions[$i];
        my ( $rxn_oid_curr, $rxn_order_curr, $rxn_name, $rxn_definition, $is_mandatory ) = split( /\t/, $r_curr );
        my $subOrderCurr = $rxnOid2SubOrder{$rxn_oid_curr};

        my $r_next;
        $r_next = $reactions[ $i + 1 ] if $i < $nReactions - 1;
        my ( $rxn_oid_next, $rxn_order_next, $rxn_name, $rxn_definition, $is_mandatory ) = split( /\t/, $r_next );
    ...
    print "<p>\n";
    print "You may select terms from the table below " . "and add them to the function cart.";
    print "</p>\n";
    ...
    foreach my $r (@reactions) {
        ...
        if ( $nCatalysts == 0 && $nLhs && $nRhs > 0 ) {
            ...
            foreach my $term_oid (@term_oids) {
                ...
                if ( !defined($n) ) {
                    webLog( "printReactionTerms: cannot find " . "term_oid=$term_oid\n" );
                    next;
                }
                my $k    = "$rxn_oid:$term_oid";
                my $type = $rxnTerm2Type{$k};
        ...
        my $c_rxn_definition = ImgPwayBrowser::getReactionCompounds( $dbh, $rxn_oid );
        my $rxn_url = "$main_cgi?section=ImgReaction" . "&page=imgRxnDetail&rxn_oid=$rxn_oid";
        if ( !blankStr($rxn_definition) ) {
            ...
            my $url0 = "$main_cgi?section=ImgTermBrowser" . "&page=imgTermDetail&term_oid=";
            print "<td class='$classStr'>\n";
            print "<div class='yui-dt-liner'>\n";
            my $s;
            foreach my $term_oid (@lhs_term_oids) {
...
sub printTermGenes {
    ...
    foreach my $term_oid (@$term_oids_aref) {
        ...
        if ($cluster_id) {
            $cur = execSql( $dbh, $sql, $verbose, $term_oid, $taxon_oid, $cluster_id );
        } else {
            $sql = qq{
                 select g.gene_oid
                 from gene_img_functions g
...
sub printBCNPForm {
    ...
    if ($upd) {
        ...
        if ( $taxon_oid && isInt($taxon_oid) ) {
            $sql .= " and np.taxon_oid = ? ";
            $cur = execSql( $dbh, $sql, $verbose, $compound_oid, $cluster_id, $taxon_oid );
        } else {
    ...
    for ( ;; ) {
        ...
        if ( $ex1 eq 'CHEBI' && isInt($ex2) ) {
            my $url3 = "https://www.ebi.ac.uk/chebi/searchId.do?chebiId=" . $ex2;
    ...
    print "<p>NCBI Accession: " . nbsp(2) . "<input type='text' name='ncbi_acc' value = '$ncbi_acc' size='20' />\n";
    print "<p>NCBI Taxon: " . nbsp(2) . "<input type='text' name='ncbi_taxon' value = '$ncbi_taxon' size='20' />\n";
...
sub printBCNPForm_old {
    ...
    if ($upd) {
        ...
        my $cur = execSql( $dbh, $sql, $verbose, $cluster_id );
        (
            $np_id,    $project_oid, $gold_id,  $np_product_name, $np_product_link, $np_type,
            $activity, $genbank_id,  $evidence, $img_compound_id, $compound_name
          )
          = $cur->fetchrow();
        $cur->finish();
    ...
    if ($img_compound_id) {
        ...
        GeneDetail::printAttrRowRaw( "IMG Compound ID", alink( $url2, $img_compound_id ) );
    }
    if ($compound_name) {
    ...
    print "<p><input type='radio' name='np_mode' value='enter' />" . nbsp(2) . "Use Product Name: \n";
    print "<input type='text' name='newNpName' value='$np_product_name' " . "size='60' maxLength='200' />\n";

    print "<p><input type='radio' name='np_mode' value='select' checked />"
      . nbsp(2)
      . "<b>(Preferred)</b> Select From IMG Compound List:<br/>\n";
...
sub dbAddNP_old {
    ...
    $cluster_id =~ s/'/''/g;    # replace ' with ''
    my $sql =
        "insert into natural_product (np_id, "
...
sub dbUpdateNP {
    ...
    my $sql = "update np_biosynthesis_source " . "set compound_oid = $compound_oid";
    if ($ncbi_acc) {
...
sub printMyIMGBCNPForm {
    ...
    if ($upd) {
        ...
        if ( $taxon_oid && isInt($taxon_oid) ) {
            $sql .= " and np.taxon_oid = ? ";
            $cur = execSql( $dbh, $sql, $verbose, $compound_oid, $cluster_id, $contact_oid, $taxon_oid );
        } else {
...
sub dbAddMyIMGNP {
    ...
    my $sql =
        "insert into myimg_bio_cluster_np (cluster_id, "
      . "taxon_oid, compound_oid, ncbi_acc, ncbi_taxon, "
...
sub dbUpdateMyIMGNP {
    ...
    my $sql = "update myimg_bio_cluster_np " . "set compound_oid = $compound_oid";
    if ($ncbi_acc) {
        $sql .= ", ncbi_acc = '" . $ncbi_acc . "'";
    } else {
        $sql .= ", ncbi_acc = null";
...
sub processBiosyntheticClusters {
    ...
    if ( $dbClusterIds_ref && scalar(@$dbClusterIds_ref) > 0 ) {
        my $cluster_ids_str = OracleUtil::getFuncIdsInClause( $dbh, @$dbClusterIds_ref );
        $db_cluster_ids_clause = " and g.cluster_id in ($cluster_ids_str) ";
    }
    ...
    if ( $metaClusterIds_ref && scalar(@$metaClusterIds_ref) > 0 ) {
        ...
        for my $metaId (@$metaClusterIds_ref) {
            ...
            $taxons_h{$taxon_oid}   = 1;
            $bcid2taxon{$metaId}    = $taxon_oid;
            $bcid2scaffold{$metaId} = $scaffold_oid;
            $bcid2geneCnt{$metaId}  = $gene_count;

            $bcid2startCoord{$metaId} = $start_coord;
    ...
    # bug abc/main.cgi?section=MeshTree&page=ecClusterList&ecId=EC:1.1.4.1
    # no vaild taxons
    ...
    #( $dbh, $db_cluster_ids_clause, $taxon_clause,
    #  \@binds, $rclause, $imgClause );

    OracleUtil::truncTable( $dbh, "gtt_func_id" )
	if ( $db_cluster_ids_clause =~ /gtt_func_id/i );
    ...
    my $hideTaxonCol = 0;
    if ( scalar(@taxon_oids) == 1 && !$frombccart) {
        my $taxon_oid  = @taxon_oids[0];
        my $taxon_name = $taxon2name_href->{$taxon_oid};
        if ( $taxon_in_file_href->{$taxon_oid} ) {
            $taxon_name = HtmlUtil::printMetaTaxonName( $taxon_oid, $taxon_name, 'assembled', 1 );
        } else {
    ...
    foreach my $cluster_id (@cluster_ids_all) {
        ...
        my $r;
        my $tmp = "<input type='checkbox' name='bc_id' value='$cluster_id' />\n";
        $r .= $sd . $tmp . "\t";

        my $url1;
        if ( WebUtil::isInt($cluster_id) ) {
            $url1 = "$section_cgi&page=cluster_detail&cluster_id=$cluster_id";
        }
        else {
            my ( $scaffold_oid, $start_coord, $end_coord ) = split( / /, $cluster_id );
        ...
        if ( $taxon_in_file_href->{$taxon_oid} ) {
            if ( !$hideTaxonCol ) {
                $taxon_name = HtmlUtil::appendMetaTaxonNameWithDataType( $taxon_name, 'assembled' );
            ...
            $s_url = "$main_cgi?section=ScaffoldDetail"
		   . "&page=scaffoldDetail&scaffold_oid=$scaffold_oid";
        }
        if ( !$hideTaxonCol ) {
        ...
        if ($genbank_id) {
            $r .= $genbank_id . $sd . alink( "${ncbi_base_url}$genbank_id", $genbank_id ) . "\t";
        } else {
    ...
    $it->printOuterTable(1);
    BcUtil::printTableFooter("processbc") if (!$frombccart);

    print end_form() if (!$frombccart);
    printStatusLine( "$cnt Biosynthetic Clusters retrieved.", 2 ) if (!$frombccart);

    return $cnt;
}

sub getBcPathwayList {
    my ( $dbh, $db_cluster_ids_clause, $taxon_clause, $binds_ref, $rclause, $imgClause ) = @_;
...
sub printBiosyntheticMetaGenes {
    ...
    if ( $data_type ne 'unassembled' ) {
        ...
        my $idClause;
        if ( scalar(@bc_ids) > 0 ) {
            my $funcIdsInClause = OracleUtil::getFuncIdsInClause( $dbh, @bc_ids );
...
sub printSimilarBCGF {
    ...
    my $hint;
    $hint =
        "The result is based on <u>Pfams of selected genes</u>. "
    ...
    if ( scalar(@clusters) > 0 ) {
        my $funcIdsInClause = OracleUtil::getFuncIdsInClause( $dbh, @clusters );
...
sub viewNeighborhoodsForSelectedClusters {
    ...
    if ( $metaClusterIds_ref && scalar(@$metaClusterIds_ref) > 0 ) {
        ...
        for my $metaId (@$metaClusterIds_ref) {
            my ( $scaffold_oid, $start_coord, $end_coord ) = split( / /, $metaId );
            my $cur = execSql( $dbh, $sql, $verbose, $scaffold_oid );
            my ( $taxon_oid, $in_file, $scf_ext_accession ) = $cur->fetchrow();
            $cur->finish();

            my ($clustergenes, $cluster_h, $scaffold_info)
            = getAllInfoForCluster($dbh, $metaId, $taxon_oid, $in_file);
    ...
    if ( $in_file eq "Yes" ) {
        ...
        foreach my $scf (@metascaffolds) {
            ...
            foreach my $line (@genes_on_s) {
                ...
                $scaffold_info{$scf} = $scf . "\t" . $taxon_oid . "\t" . $topology . "\t" . $scf_length;
        ...
        if ( $cluster_id ) {
            if ( WebUtil::isInt($cluster_id) ) {
                ...
                $cur = execSql( $dbh, $sql, $verbose, $id2 );
            }
            else {
                #use: "and ((g.end_coord > ? and g.end_coord <= ?) or (g.start_coord >= ? and g.start_coord < ?))"
                #instead of "and g.start_coord >= ? and g.end_coord <= ?"
...
sub printPfamDomain {
    ...
    foreach my $g2 (@batch_genes) {
        ...
        foreach my $bc2 (@sim_bc) {
            my %match_genes = findBCGenesWithPfams( $dbh, $bc2, \@pfams );
            print "<td class='img'>\n";
            ...
            foreach my $k2 (@keys_ordered) {
                ...
                if ( isInt($k2) ) {
                    my $locus_cur = execSql( $dbh, $locus_tag_sql, $verbose, $k2 );
                    ($locus_tag) = $locus_cur->fetchrow();
                    $locus_cur->finish();
                    my $g_url = "$main_cgi?section=GeneDetail&page=geneDetail" . "&gene_oid=$k2";
...
sub findGenePfams {
    ...
    my @pfams = ();
    if ( isInt($g2) ) {
        my ( $sql2, @bindList ) = FunctionAlignmentUtil::getPfamSqlForGene( $g2, undef, "", "" );
        my ( $cnt, $recs_ref, $doHmm ) = FunctionAlignmentUtil::execPfamSearch( $dbh, $sql2, \@bindList );
