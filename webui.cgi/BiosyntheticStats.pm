...
sub dispatch {
    ...
    if ( $page eq "stats" ) {
        ...
        if ($precomputed) {
            ...
            print "<input type='button' class='medbutton' " . "value='Display' onclick=\"$func\" />";
        }
        print "</span>";
...
sub printOverview {
    ...
    my @tabIndex = (
        "#biostatstab1", "#biostatstab2", "#biostatstab3", "#biostatstab4", "#biostatstab5", "#biostatstab6",
        "#biostatstab7", "#biostatstab8", "#biostatstab9"
    );
    ...
    if ($precomputed) {
        ...
        print
"<p>Please click <font color='blue'><u>Display</u></font> to load the distribution of Biosynthetic Clusters by Length.</p>";
        print "<input type='button' class='medbutton' " . "value='Display' onclick=\"$func\" />";
    }
    print "</span>";
    ...
    if ($precomputed) {
        ...
        print
"<p>Please click <font color='blue'><u>Display</u></font> to load the distribution of Biosynthetic Clusters by Gene Count.</p>";
        print "<input type='button' class='medbutton' " . "value='Display' onclick=\"$func\" />";
    }
    print "</span>";
...
sub printStats {
    ...
    my ( $npTypeExpCnt, $npTypePredCnt, $npTypeUnknownCnt, $npIdExpCnt, $npIdPredCnt, $npIdUnknownCnt ) = getStatsByNp($dbh);
...
sub printStatsHomePage {
    ...
    my $sd = $it->getSdDelim();
    $it->addColSpec( "<b>Biosynthetic Cluster (BC) Statistics</b>" );
    $it->addColSpec( "<b>Number</b>", '', "right" );

    my $row = "$indent Total\t$totalCnt\t";
    $it->addRow($row);

    if ( $withGBcnt > 0 ) {
        my $url = "$section_cgi&page=withGenbankID";
        my $row = $indent . $indent . "with Genbank ID\t". alink( $url, $withGBcnt ) ."\t";
        $it->addRow($row);
    }
    my $row = $indent . $indent . "by Domain\t". "&nbsp;\t";
    $it->addRow($row);
    ...
    foreach my $domain ( sort( keys %domain2cnt ) ) {
        ...
        my $row = $indent . $indent . $indent . "in $domain_name\t". alink( $url, $cluster_cnt ) ."\t";
        $it->addRow($row);
    }
    ...
    my $url_byNpId = "$section_cgi&page=byNpId";
    my $byNaturalProduct = alink( $url_byNpId, "by Secondary Metabolite" );
    my $row = $indent . $indent . "$byNaturalProduct\t". "&nbsp;\t";
    $it->addRow($row);

    my $row = $indent . $indent . $indent . "with Experimentally Verified Secondary Metabolite\t". alink( "$url_byNpId&evidence=Experimental", $npIdExpCnt ) ."\t";
    $it->addRow($row);


    if ($npIdPredCnt) {
        my $row = $indent . $indent . $indent . "with Predicted Secondary Metabolite\t". alink( "$url_byNpId&evidence=Predicted", $npIdPredCnt ) ."\t";
        $it->addRow($row);
    }


    my $row = $indent . $indent . $indent . "with No Secondary Metabolite\t";

    if ($npIdUnknownCnt) {
        $row .= alink( "$url_byNpId&np=$unknown", $npIdUnknownCnt ). "\t";
    } else {
        $row .= "$npIdUnknownCnt\t";
    }
    $it->addRow($row);
...
sub printClustersWithGenbankID {
    ...
    if ( scalar(@$clusters_aref) > 0 ) {
        my $cluster_ids_str = OracleUtil::getFuncIdsInClause( $dbh, @$clusters_aref );
    ...
    my $it = new InnerTable( 1, "bcwgenbank$$", "bcwgenbank", 1 );
    my $sd = $it->getSdDelim();
    ...
    for ( ; ; ) {
        ...
        my $url   = "$main_cgi?section=BiosyntheticDetail" . "&page=cluster_detail&cluster_id=$bc_id";
...
sub printStatsByBCType {
    ...
    if ( scalar(@ecluster_ids) > 0 ) {
        my $cluster_ids_str = OracleUtil::getFuncIdsInClause( $dbh, @ecluster_ids );
    ...
    for ( ; ; ) {
        ...
        if ($total_clusters) {
            $link = alink( $url_section . "&bc_type=$bct", $total_clusters, "_blank" );
        }
        ...
        $link = $expcnt;
        if ($expcnt) {
            $link = alink( $url_section . "&bc_type=$bct&evidence=Experimental", $expcnt, "_blank" );
        }
        $row .= $expcnt . $sd . $link . "\t";
        ...
        $link = $pcnt;
        if ($pcnt) {
            $link = alink( $url_section . "&bc_type=$bct&evidence=Predicted", $pcnt, "_blank" );
        }
        $row .= $pcnt . $sd . $link . "\t";
...
sub printStatsByNp {
    ...
    foreach my $np ( keys %done ) {
        ...
        if ( $st == 0 ) {    # add colored link -anna
            my $imageref = "<img src='$tmp_url/" . $chart->FILE_PREFIX . "-color-" . $cnt . ".png' border=0>";
            $r = escHtml($np) . $sd . alink( "$url&np=$np&evidence=Experimental", $imageref, "", 1 );
            $r .= "&nbsp;&nbsp;";
        }
        if ( !$isType && WebUtil::isInt($np) ) {
            my $npid_url = "main.cgi?section=ImgCompound" . "&page=imgCpdDetail&compound_oid=$np";
...
sub printClustersByNp {
    ...
    BiosyntheticDetail::processBiosyntheticClusters( $dbh, '', \@clusters, '', $title, $subTitle );
}
...
sub printStatsByProbability {
    ...
    for ( ; ; ) {
        ...
        my $row;
        $row = $sd . "<input type='radio' name='probability' value='$prb'/>\t"
          if $dofilter ne "";
        $row .= $prb . "\t";

        my $link = $total_clusters;
        if ( $total_clusters && $total_clusters < $UPPER_LIMIT ) {
            $link = alink( $url_section . "&probability=$prb", $total_clusters, "_blank" );
        }
        $row .= $total_clusters . $sd . $link . "\t";
...
sub printStatsByGeneCount {
    ...
    if ( $exp_cnt > 0 ) {
        ( $items_aref, $data_aref ) = binIt( $numbins, $lo, $hi, \%e_clust2cnt, 0 );
    }
    ...
    if ( $pred_cnt > 0 ) {
        ( $items_aref, $data_aref ) = binIt( $numbins, $lo, $hi, \%p_clust2cnt, 0 );
    }
    ...
    my $url_section =
      "$section_cgi&page=clustersByGeneCount" . "&probability=$probability&phylum=" . WebUtil::massageToUrl2($phylum);
    ...
    for ( my $i = 0 ; $i < $size ; $i++ ) {
        ...
        my $row;
        $row = $sd . "<input type='radio' name='gene_count' value='$key'/>\t"
          if $dofilter ne "";
        ...
        if ( $pred_cnt > 0 ) {
            ...
            if ( $cnt && $cnt < $UPPER_LIMIT ) {
                $link = alink( $url_section . "&attr=Predicted&gene_count=$key", $cnt, "_blank" );
            }
            $row .= $cnt . $sd . $link . "\t";
        }
        ...
        if ( $exp_cnt > 0 ) {
            ...
            my $link = $cnt;
            if ( $cnt && $cnt < $UPPER_LIMIT ) {
                $link = alink( $url_section . "&attr=Experimental&gene_count=$key", $cnt, "_blank" );
            }
            $row .= $cnt . $sd . $link . "\t";
        }
...
sub printMyChart {
    my (
        $items_aref, $data_aref, $title,   $yaxis,       $url,       $series,
        $name,       $param,     $use_log, $orientation, $log_scale, $is_table
      )
      = @_;
        ...
        && scalar @datas > 0 )
    {
        ...
        print "$title<br/>";
        ChartUtil::printBarChart( $name, $param, $yaxis, $url, \@items, \@sections, \@datas, $series, $use_log, $orientation,
            $log_scale );
...
sub printBreakdownBy {
    ...
    if ( $type eq "length" && $precomputed ) {
        ...
        printMyChart( $e_items_aref, $e_data_aref, $title, $yaxis, $url . "&attr=Experimental",
            \@series, $name, $param, $use_log, $orientation, $log_scale, 0 );

        my $title = "Predicted Clusters";
        printMyChart( $p_items_aref, $p_data_aref, $title, $yaxis, $url . "&attr=Predicted",
            \@series, $name, $param, $use_log, $orientation, $log_scale, 0 );
        print "</p>";
        return;
        ...
        $title = "Experimentally verified Clusters";
        printMyChart( $e_items_aref, $e_data_aref, $title, $yaxis, $url . "&attr=Experimental",
            \@series, $name, $param, $use_log, $orientation, $log_scale, 1 );
        ...
        ChartUtil::printPieChart( "Pfam", "func_code", $url, $e_items_aref, $e_sections_aref, $e_data_aref, \@series, 1,
            $UPPER_LIMIT, "ebc" );
        ...
        ChartUtil::printPieChart( "Pfam", "func_code", $url, $items_aref, $sections_aref, $data_aref, \@series, 1,
            $UPPER_LIMIT, "pbc", 0 );
        return;
    }
    ...
    if ( $type eq "domain" ) {
        ...
        $log_scale = 1;
        for ( ; ; ) {
            my ( $domain, $phylum, $ir_class, $total_clusters ) = $cur->fetchrow();
            last if !$total_clusters;
        ...
        my ( $items_aref, $data_aref ) = binIt( 25, $lo, $hi, \%clust2len_part, $use_log );
            ...
            && scalar @datas > 0 )
        {
            ...
            ChartUtil::printBarChart(
                $name,   $param,   "Number of BCs", $url,         \@items, \@sections,
                \@datas, \@series, $use_log,        $orientation, $log_scale
            );
            print "<br/>";
        }
        ...
        my ( $items_aref, $data_aref ) = binIt( 25, $e_lo, $e_hi, \%e_clust2len, $use_log );
            ...
            && scalar @datas > 0 )
        {
            ...
            ChartUtil::printBarChart(
                ...
                \@items, \@sections, \@datas, \@series, $use_log, $orientation, $log_scale
            );
            print "<br/>";
        }
        ...
        my ( $items_aref, $data_aref ) = binIt( 25, $lo, $hi, \%p_clust2len_part, $use_log );
        ...
        my $numbins = 10;
        $numbins = 25 if $hi > 25000;
        my ( $items_aref, $data_aref ) = binIt( $numbins, $lo, $hi, \%clust2cnt, $use_log );
            ...
            && scalar @datas > 0 )
        {
            ...
            print "Total Clusters<br/>";
            ChartUtil::printBarChart(
                $name,   $param,   "Number of BCs", $url,         \@items, \@items,
                \@datas, \@series, $use_log,        $orientation, $log_scale
            );
            print "</td>";
        }
        ...
        my ( $items_aref, $data_aref ) = binIt( $numbins, $e_lo, $e_hi, \%e_clust2cnt, $use_log );
            ...
            && scalar @datas > 0 )
        {
            ...
            ChartUtil::printBarChart(
                ...
                "Number of BCs",
                $url . "&attr=Experimental",
                \@items, \@items, \@datas, \@series, $use_log, $orientation, $log_scale
            );
            print "</td>\n";
        }
        ...
        my ( $items_aref, $data_aref ) = binIt( $numbins, $p_lo, $p_hi, \%p_clust2cnt, $use_log );
        ...
        ChartUtil::printPieChart( "Pfam", "func_code", $url, \@items, \@sections, \@data, \@series, 1, $UPPER_LIMIT, "ebc" );
        ...
        ChartUtil::printPieChart( "Pfam", "func_code", $url, \@items, \@sections, \@data, \@series, 1, $UPPER_LIMIT, "pbc",
            0 );
        return;
        ...
        ChartUtil::printPieChart( "KEGG", "category", $url, \@items, \@items, \@data, \@series, 0, $UPPER_LIMIT, "bc" );
        return;
    }
    ...
    ChartUtil::printBarChart( $name, $param, "Number of BCs",
        $url, \@items, \@sections, \@datas, \@series, $use_log, $orientation, $log_scale );
}
...
sub printStatsByPhylum {
    ...
    my $url_section = "$section_cgi&page=clustersByPhylum" . "&gene_count=$gene_count&probability=$probability";
    my $cnt         = 0;
    my $name        = "Phylum";
    my $param       = "phylum";
    my @series      = ("count");
    ...
    for ( ; ; ) {
        ...
        my $row;
        $row = $sd . "<input type='radio' name='phylum' value='$phylum'/>\t"
          if $dofilter ne "";
        ...
        my $link = $total_clusters;
        if ( $total_clusters && $total_clusters < $UPPER_LIMIT ) {
            $link = alink( $url_section . "&phylum=" . WebUtil::massageToUrl2($phylum), $total_clusters, "_blank" );
        }
        $row .= $total_clusters . $sd . $link . "\t";
        ...
        && scalar @data > 5 )
    {
        ...
        my $log_scale   = 1;
        ChartUtil::printBarChart(
            $name,   $param,   "Number of BCs", $url_section, \@items, \@sections,
            \@datas, \@series, $use_log,        $orientation, $log_scale
        );
    }
    print "</td></tr>\n";
...
sub printClustersByProbability {
    ...
    print start_form(
        -id     => "bcbyprobability_frm",
        -name   => "mainForm",
        -action => "$main_cgi"
    );

    my $it = new InnerTable( 1, "bcbyprobability$$", "bcbyprobability", 1 );
    my $sd = $it->getSdDelim();
    $it->addColSpec( "Select" );
    $it->addColSpec( "Cluster ID",  "asc", "right" );
    $it->addColSpec( "Genome Name", "asc", "left" );
...
sub printClustersByPhylum {
    ...
    if ( $dm eq "*Microbiome" ) {
        ...
        # metagenomes are passed one level deeper
        $taxonClause = " and tx.domain = ? and tx.phylum = ? and tx.ir_class = ? ";
        $phylumStr   = $dm . ":" . $ph . ":" . $irc;
    ...
    my $bc2evidence_href = getBC2Evidence( $dbh, $rclause, $imgClause, $clusterClause );
    ...
    my $it = new InnerTable( 1, "bcbyphylum$$", "bcbyphylum", 1 );
    my $sd = $it->getSdDelim();
    $it->addColSpec( "Select" );
    $it->addColSpec( "Cluster ID",  "asc", "right" );
    $it->addColSpec( "Genome Name", "asc", "left" );
...
sub printClustersByPhylo {
    ...
    for ( ; ; ) {
        ...
        my $url =
          "$main_cgi?section=BiosyntheticDetail" . "&page=cluster_detail&taxon_oid=$taxon_oid" . "&cluster_id=$cluster_id";
        my $txurl = "$main_cgi?section=TaxonDetail" . "&page=taxonDetail&taxon_oid=$taxon_oid";
...
sub printClustersByLength {
    ...
    if ( $count > $UPPER_LIMIT ) {
        ...
        my ( $items_aref, $data_aref ) = binIt( 25, $min, $max, \%clust2len, $use_log );
        my @items    = @$items_aref;
        my @data     = @$data_aref;
        my $datastr  = join( ",", @data );
        my @datas    = ($datastr);
...
sub printClustersByGeneCount {
    ...
    my $clusterClause = getClusterClause( $probability, "", $phylum, \@clusters );
...
sub getClusterClause {
    ...
    if ( $phylum ne "" ) {
        ...
        if ( $dm eq "*Microbiome" ) {
            ...
            # metagenomes are passed one level deeper
            $taxonClause = " and tx.domain = '$dm'" . " and tx.phylum = '$ph'" . " and tx.ir_class = '$irc' ";
        }
    ...
    if ( scalar(@ids) > 0 || $cluster_aref ne "" ) {
        ...
        my $funcIdsInClause = OracleUtil::getFuncIdsInClause( $dbh, @ids );
...
sub printPfamList {
    ...
    for ( ; ; ) {
        my ( $name, $ext_accession, $description, $db_source, $cluster_cnt ) = $cur->fetchrow();
        last if !$name;

        my $r;
        $r .= $sd . "<input type='checkbox' name='func_id' value='$ext_accession'/>\t";
        $r .= "$ext_accession\t";
