...
sub getPageTitle {  
    return 'Biosynthetic Cluster Statistics';
}
...
sub printStatsHomePage {
    ...
    my $row = $indent . $indent . $indent . "with Experimentally Verified Secondary Metabolite\t"
	. alink("$url_byNpId&evidence=Experimental", $npIdExpCnt) ."\t";
    $it->addRow($row);

    if ($npIdPredCnt) {
        my $row = $indent . $indent . $indent . "with Predicted Secondary Metabolite\t"
	    . alink("$url_byNpId&evidence=Predicted", $npIdPredCnt) ."\t";
        $it->addRow($row);
    }
...
sub printBreakdownBy {
    ...
    printStatsJS(); # moved here because no more tabs -anna
