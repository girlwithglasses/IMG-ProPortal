...
sub initialize {
	...
	if ( @_ ) {
		...
		warn "No session supplied!" unless exists $args->{session};
		$cgi = $args->{cgi} if $args->{cgi};
		$cookie_name = $args->{cookie_name} if $args->{cookie_name};
		$g_session = $args->{session};
	}
	else {
		warn "Running WebUtil initialise";
		# don't run in Dancer2 / PSGI environment
#		return if $ENV{ PLACK_ENV };
...
sub createTmpIndex {
	...
	return;
    return if $env->{dev_site} || $ENV{TESTING};
...
############################################################################
# equivalent of $app->user->can_edit('gene-level')
sub canEditGeneTerm {
    my ( $dbh, $contact_oid ) = @_;
    return 0 if !$contact_oid;
...
############################################################################
# equivalent of $app->user->can_edit('img-pathway')
sub canEditPathway {
    my ( $dbh, $contact_oid ) = @_;
    return 0 if !$contact_oid;
...
# canEditBin
############################################################################
# equivalent of $app->user->can_edit('img-bin')
...
sub get_dir {
	my $d_type = shift;
	my ($dir, $sess_id) = getSessionDir() . "/" . $d_type;
	my $err;
	make_path( $dir, { error => \$err } ) unless -e $dir;
	webError( @$err ) if $err;
	return ( $dir, $sess_id );
}

sub getGenomeHitsDir {
	return get_dir('genomeHits');
    my $sessionId = getSessionId();
    my $dir       = getSessionDir() . "/genomeHits";
	local $@;
	eval { make_dir( $dir ); };
	webError( $@ ) if $@;
