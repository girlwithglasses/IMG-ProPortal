...
#     -- anna 6/25/2012
# $Id: King.pm 35860 2016-07-12 19:07:46Z ewandersen $
############################################################################
package King;
...
sub writeKingApplet {
    print qq{
      <h2>Under Construction - writeKingApplet</h2>
    };
}
...
sub getPlotDataJSON {
    ...
    if ($set2scafs_href) {
	$scafSets = "\"scaffold_sets\": [";
        foreach my $x (keys %$set2scafs_href) {
            my $url = "$main_cgi?section=WorkspaceScafSet"
		. "&page=showDetail&filename=$x&folder=scaffold";
	    my $link = alink($url, $x, "_blank");
	    $scafSets .= "{\"name\":\"$x\",\"url\":\"$url\"},";
    ...
    while( my $s = $rfh->getline() ) {
        ...
        my ($start, $end) = split("-", $range);
        my $length = $end+30000; 
        my $surl = "$main_cgi?section=ScaffoldGraph&color=cog"
                 . "&start_coord=$start&end_coord=$end"
                 . "&seq_length=$length&scaffold_oid=$id";
    ...
    my $axes = "\"axes\": [ $max_pc1,0.000,0.000,0.000,$max_pc2,0.000,0.000,0.000,$max_pc3 ]";

    my $jsondata = "{".$cartlink.",".$axes.",".$axesLabels;
...
############################################################################
# printPlotElements - prints the tags for ImgPlotter to plot
############################################################################
sub printPlotElements {
  my( $pca_data ) = @_;
  ...
  print qq{
    ...
    <table>
      <tr>
        ...
        <td valign=top>
          <button type='button' onclick='window.KmerUI_addToCart();'>Add Selected to Cart</button>
          <div id="legend"></div>
        </td>
      </tr>
    </table>
    ...
    <script src='/js/KmerUI.js'></script>
  };
}
1;
