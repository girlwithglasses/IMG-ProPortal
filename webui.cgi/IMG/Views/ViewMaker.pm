...
#	say "dsl: " . Dumper $dsl;
	...
	if ( $part ) {
		if ( 'page_top' eq $part ) {
			# just print the top of the page
			return print_app_header( $args{tmpl_args} );
		}
		elsif ( 'page_bottom' eq $part ) {
			# just the page bottom
			my $out = printContentEnd();
			$out .= printMainFooter( $args{tmpl_args} );
			return $out;
		}
	}
...
#	printContentHome() if 'Home' eq $args{current};
	...
	if ( $args{current} eq "Home" ) {
		if ( $env->{abc} ) {
			$out .= printContentHome() . abc_home();
		}
		elsif ( $env->{proportal} ) {
			$out .= printContentHome() . proportal_home();
		}
...
sub proportal_home {
	...
	if ( ! $section ) {
		...
		# home page url
		my $class = $req->param('class') || 'datamart';
...
#	printErrorDiv();
	...
	$out .= printStatsTableDiv( $maxAddDate, $maxErDate );
	$out .= printContentHome();
	my $templateFile = $env->{base_dir} . "/home-v33.html";
	my $hmpGoogleJs = "";
...
#				$hmpGoogleJs = file2Str( $f, 1 );
	...
	if ( $env->{include_metagenomes} ) {
		...
		my $img_app_term = get_imgAppTerm();
...
#		$table_str = file2Str( $file, 1 );
	...
	elsif ( ! $env->{user_restricted_site} && ! $env->{img_hmp} ) {
...
#    $template->param( googleStr    => $googleStr );
	...
	my $data = {
		title => $args{title},
		cfg   => $env,
		appname => get_imgAppTerm(),
		logofile => $logofile,
	};
	$data->{cfg}{yui} = $env->{yui_dir_28};
	if ( $self->has_user ) {
	...
	# NEED NUMBER OF TAXA!!
	if ($args{n_taxa}) {
		$data->{n_taxa} = $args{n_taxa};
	}

	if ( $args{include_scripts}) {
		push @{$data->{tmpl_includes}{scripts}}, $args{include_scripts};
	}
	if ( $args{include_styles} ) {
		push @{$data->{tmpl_includes}{styles}}, $args{include_styles};
	}
	if ( $args{yui_js} ) {
	...
	if ( $env->{enable_autocomplete} ) {
		...
		push @{$data->{tmpl_includes}{scripts}}, 'autocomplete.tt';
		# https://localhost/~kchu/preComputedData/autocompleteAll.php
		$data->{autocomplete_url} = $env->{top_base_url} . "api/";
		if ( $env->{include_metagenomes} ) {
			$data->{autocomplete_url} .= 'autocompleteAll.php';
		}
		else {
			$data->{autocomplete_url} .= 'autocompleteIsolate.php';
		}
	}
...
<div id="jgi-logo">
<a href="http://jgi.doe.gov/" title="DOE Joint Genome Institute - } . get_imgAppTerm() . qq{">
<img width="480" height="70" src="$env->{top_base_url}/images/$logofile" alt="DOE Joint Genome Institute's } . get_imgAppTerm() . qq{ logo"/>
</a>
</div>
};
...
#	my $contact_oid = getContactOid();
	my $contact_oid = ( $user ) ? $user->contact_oid : 0;
	my $isEditor    = ( $user ) ? $user->is_editor : 0;
	my $super_user  = ( $user ) ? $user->is_superuser : 0;
#	my $isEditor = $self->user->is_editor;
...
#	my $super_user = getSuperUser();
	...
	$template->param( imgAppTerm   => get_imgAppTerm() );
	...
	if ( $env->{enable_mybin} && $user && $user->can_edit('img-bin') ) {
#	canEditBin( $dbh, $contact_oid ) ) {
...
#	my $img            = $cgi->param("img");
	...
	# no exit read
	my $buildDate = 'unknown'; # file2Str( $env->{base_dir} . "/buildDate", 1 );
...
sub printBreadcrumbsDiv {
	...
	my %breadcrumbs = (
		...
		CompareGenomes   => "Compare Genomes",
		AnaCart          => "Analysis Cart",
		MyIMG            => "My " . get_imgAppTerm(),
		about            => "Using ". get_imgAppTerm(),
		ImgStatsOverview => "IMG Stats Overview",
		IMGContent       => "IMG Content",
		RNAStudies       => "RNASeq Studies",
		Methylomics      => "Methylomics Experiments",
...
#	}
	...
	my $lastCart = $self->session->read("lastCart") || 'geneCart';
	if (
		! $isEditor
...
"<td style='line-height: 1.25em; width: 90px;'>$domain_name</td>\n";
			$out .= "<td style='line-height: 1.25em;' align='right'>"
...
<br>
	...
	<th align="right">Total</th>
		...
		$out .= qq{
	 <h2>} . get_imgAppTerm() . qq{ Content</h2>
	 <table cellspacing="0" cellpadding="0">
	 <tr>
		 <th align="right" colspan="2" > &nbsp; </th>
...
#		my $contact_oid = getContactOid();
		...
		my $contact_oid = $user ? $user->contact_oid : undef;

		return if ! $contact_oid || $req->param("logout") ne "";

		my $name = $user->name;

#		my $name = WebUtil::getUserName2() || WebUtil::getUserName();

		my $tmp = $env->{oldLogin} ? '' : '<br> (JGI SSO)';

		return { name => $name };

		return qq{
	<div id="login">
		Hi $name &nbsp; | &nbsp; <a href="logout"> Logout </a>
		$tmp
	</div>
	};
...
sub webError {
    ...
    my $buildDate = $env->{buildDate} || 'unknown';  #file2Str( $env->{base_dir} . "/buildDate", 1 );
...
sub get_autocomplete_file {
	if ( $env->{user_restricted_site} && $env->{enable_workspace} ) {
...
sub get_imgAppTerm {
	...
	return $imgAppTerm if $imgAppTerm;
