############################################################################
#	IMG::App::Dispatcher.pm
#
#	Parse query params and run the appropriate code
#
#	$Id: Dispatcher.pm 34173 2015-09-03 19:38:35Z aireland $
############################################################################
...
#	say 'request: ' . Dumper $req;
	...
	my $section_table = {
        AbundanceProfileSearch => sub {
			...
			$tmpl_args{help} .= ( $self->config->{include_metagenomes} )
            ? "19"
            : "51";
        },
        StudyViewer => sub {
        ...
        EgtCluster => sub {
            ...
            $tmpl_args{help} = "DistanceTree.pdf#page=5" if defined $req->params->{method} && $req->params->{method} eq 'hier';
        },
        EmblFile => sub {
        ...
        FindGenes => sub {
            ...
            if (   $page eq 'findGenes'
                || $page eq 'geneSearch'
                || ( $page ne 'geneSearchForm' && ! paramMatch( $req, "fgFindGenes") ) ) {
        ...
        FuncCartStor => sub {
            ...
            if ( $page eq 'funcCart' && $self->config->{enable_genomelistJson} ) {
            ## Eh?!?!
        ...
        Kmer => sub {
            %tmpl_args = ( title => "Kmer Frequency Analysis", current => "FindGenomes" ) if ! paramMatch( $req, "export");
        },
        GenBankFile => sub {
        ...
        GeneCartStor => sub  {
            ...
            my $last_cart = ( paramMatch( $req, 'addFunctionCart') )
            ? 'funcCart'
            : 'geneCart';
        ...
        ImgStatsOverview => sub {
            ...
            if ( $req->params->{excel} && 'yes' eq $req->params->{excel} ) {
            	$tmpl = 'excel';
            	%tmpl_args = ( filename => "stats_export$$.xls" );
            	$hdrs = sub {
            		return IMG::Views::ViewMaker::print_excel_header("stats_export$$.xls");
        ...
        ScaffoldCart => sub {
            ...
            if (   paramMatch( $req, "exportScaffoldCart")
                || paramMatch( $req, "exportFasta") ) {
            	# export excel
                $self->session->write( "lastCart", "scaffoldCart" );
            }
            elsif ( ! paramMatch( $req, "addSelectedToGeneCart") ) {
                $self->session->write( "lastCart", "scaffoldCart" );
            }
        },
        GenomeCart => sub {
