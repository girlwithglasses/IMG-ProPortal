...
subtest 'valid jgi session' => sub {
	...
	$obj = IMG::App::Role::UserObj->new( config => { one => 'two' } );
	throws_ok { $obj->get_jgi_user_json(0) } qr[Missing required config param sso_api_url];

	$obj = IMG::App::Role::UserObj->new( config => { sso_url_prefix => 'https://', sso_domain => 'example.com/' });
...
#	throws_ok {  } qr[  ];
	...
	# load the test db
	$obj = IMG::App::Role::UserObj->new( config => test_config() );
