...
#        }
	...
	## print reannotation genome comparison
	printReannotationComparison($dbh, $taxon_oid, $total);
    }
...
sub printReannotationComparison {
    my ($dbh, $taxon_oid, $total) = @_;
	...
	## for now, we show this to super users only
	return;
    }
    ...
    ## old genome(s)
    my $sql = qq{
        select mtp.old_taxon_oid, t.taxon_display_name
        from map_taxon_pairs mtp, taxon t
        where new_taxon_oid = ?
        and mtp.old_taxon_oid = t.taxon_oid
        $rclause
        $imgClause
        };
    ...
    my $cur = execSql( $dbh, $sql, $verbose, $taxon_oid );
    for (;;) {
	my ($old_taxon_oid, $old_taxon_name) = $cur->fetchrow();
	last if ! $old_taxon_oid;

	print qq{
              <tr class='highlight'>
              <th class='subhead'>Re-annotation Genome Comparison</th>
              <th class='subhead'></th>
              <th class='subhead'></th>
              </tr>
            };

        my $url = "$main_cgi?section=TaxonDetail&taxon_oid=$old_taxon_oid";
        $url = alink( $url, $old_taxon_name );

	my $indent = nbsp(4);
	print "<tr class='img'>\n";
	print "<td class='img' colspan='3'>$indent Old Genome: $url</th>\n";
	print "</tr>\n";
	...
	for my $method ( 'md5dna', 'ublast' ) {
	    ...
	    print "<tr class='img'>\n";
	    my $label = $method;
	    if ( $label_mapping{$method} ) {
		$label = $label_mapping{$method};
	    }
	    print "<td class='img'>$indent $label</th>\n";
	    my $url2 = "$section_cgi&page=$method&taxon_oid=$taxon_oid";
	    $url2 = alink( $url2, $new_match );
	    if ( $new_match ) {
		print "<td class='img' align='right'>$url2</td>\n";
	    }
	    else {
		print "<td class='img' align='right'>$new_match</td>\n";
	    }
	    print "<td class='img' align='right'>$pc\%</td>\n";
	    print "</tr>\n";
	}   # end method
    ...
    ## re-annotated genome(s)
    $sql = qq{
        select mtp.new_taxon_oid, t.taxon_display_name
        from map_taxon_pairs mtp, taxon t
        where old_taxon_oid = ?
        and mtp.new_taxon_oid = t.taxon_oid
        $rclause
        $imgClause
        };
    ...
    for (;;) {
	my ($new_taxon_oid, $new_taxon_name) = $cur->fetchrow();
	last if ! $new_taxon_oid;
	...
	print qq{
        ...
        my $url = "$main_cgi?section=TaxonDetail&taxon_oid=$new_taxon_oid";
        $url = alink( $url, $new_taxon_name );

	my $indent = nbsp(4);
	print "<tr class='img'>\n";
	print "<td class='img' colspan='3'>$indent Re-annotated Genome: $url</th>\n";
	print "</tr>\n";
	...
	for my $method ( 'md5dna', 'ublast' ) {
	    ...
	    print "<td class='img'>$indent $label</th>\n";
	    my $url2 = "$section_cgi&page=$method&taxon_oid=$taxon_oid&old=1";
	    $url2 = alink( $url2, $old_match );
	    if ( $old_match ) {
		print "<td class='img' align='right'>$url2</td>\n";
	    }
	    else {
		print "<td class='img' align='right'>$old_match</td>\n";
	    }
	    print "<td class='img' align='right'>$pc\%</td>\n";
	    print "</tr>\n";
	}
