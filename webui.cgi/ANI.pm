# $Id: ANI.pm 34123 2015-08-27 21:32:44Z klchu $
package ANI;
use strict;
use CGI qw( :standard);
use Data::Dumper;
...
sub drawCliqueGroups {
    ...
    $hint .=
"Any two genomes in a clique group will be connected by an edge in the graph if they have <br/>an ANI >= 96.5 and an AF >= 0.6.";
    printHint($hint);
    print "<br/>";
...
sub printCliqueInfoForGenome {
    ...
    my ( $anicnt, $cliqueIdTaxonCnt_href, $cliqueId2Type_href, $cliqueId2GenusSpecies_href ) =
      getInfoForGenome( $dbh, $taxon_oid );
    return if $anicnt < 1;
    ...
    foreach my $clique_id ( sort { $a <=> $b } keys %$cliqueIdTaxonCnt_href ) {
        ...
        my $contSpeciesUrl = "$section_cgi&page=speciesForGenomeForClique" . "&taxon_oid=$taxon_oid&clique_id=$clique_id";
...
sub printQuickSearch {
    ...
    print "<p><font color='#003366'>" . "Please select 2 genomes:" . "</font>\n";
...
sub printUploadForm {
    ...
    print "<p><font color='#003366'>" . "Please select up to 100 genomes:" . "</font>\n";
    ...
    GenomeListJSON::printAutoComplete( "myGenomeSearchInput" . "upload_frm", "myGenomeSearchContainer" . "upload_frm" );
    GenomeListJSON::printMySubmitButton( 'go', $name, 'ANI', '', $section, 'doPairwiseWithUpload', 'meddefbutton',
        'upload_frm' );
...
sub selectFiles {
    ...
    WorkspaceUtil::printSetMainTableNoFooter( "WorkspaceGenomeSet", $workspace_dir, $sid, $folder, \@files )
      ;    # modify to have no footer
    print end_form();
...
sub printAdvancedSearchForm {
    ...
    print "<p><font color='#003366'>" . "Please select up to $max_pairwise genomes:" . "</font>";
    ...
    GenomeListJSON::printAutoComplete( "myGenomeSearchInput" . "advanced_frm", "myGenomeSearchContainer" . "advanced_frm" );
    my $button =
      GenomeListJSON::printMySubmitButtonXDiv( 'go', $name, 'ANI', '', $section, 'doPairwise', 'meddefbutton',
...
sub printPairwiseTable {
    ...
    WebUtil::printHeaderWithInfo( $title, $description, "show info for this tool", "Pairwise ANI Info", 0, "ANI.pdf" );
...
sub computePairwiseANI {
    ...
    foreach my $tx1 (@oids1) {
        ...
        for ( ; ; ) {
            ...
            my $rec = $genome1 . "\t" . $genome2 . "\t" . $ani1 . "\t" . $ani2 . "\t" . $af1 . "\t" . $af2 . "\t" . $bbh_cnt;
            push @dataRecs, $rec;
    ...
    foreach my $tx2 (@oids2) {
        ...
        for ( ; ; ) {
            ...
            # flip the values, too:
            my $rec = $genome2 . "\t" . $genome1 . "\t" . $ani2 . "\t" . $ani1 . "\t" . $af2 . "\t" . $af1 . "\t" . $bbh_cnt;
            push @dataRecs, $rec;
    ...
    foreach my $tx1 (@oids1) {
        foreach my $tx2 (@oids2) {
            ...
            while ( my $s = $rfh->getline() ) {
                ...
                my (
                    $genome1x,    $genome2x,    $size1,     $size2,     $csize,    $total_bbh, $valid_bbh,
...
sub printAllSpeciesInfo {
    ...
    foreach my $item ( sort keys %genus_species2txs ) {
        my $row;
        $row = $sd . "<input type='checkbox' name='genus_species' " . " value='$item' />\t" if $show_cb;
...
sub printGenomesForGenusSpecies {
    ...
    for ( ; ; ) {
        my ( $taxon_oid, $taxon_name, $species, $domain, $phylum, $class, $order, $family, $clique_id ) = $cur->fetchrow();
        last if ( !$taxon_oid );
        ...
        my $lineage = $domain . "\t" . $phylum . "\t" . $class . "\t" . $order . "\t" . $family . "\t" . $genus0;
...
sub getLineage {
    ...
    my @taxons;
    for ( ; ; ) {
        my ( $taxon_oid, $taxon_name, $species, $domain, $phylum, $class, $order, $family ) = $cur->fetchrow();
        last if ( !$taxon_oid );
...
sub printSpeciesForGenomeForClique {
    ...
    my $cliqueLink = alink( $cliqueUrl, $cliqueId );
    my $note       = "<p><u>Genome</u>: $taxon_link" . "<br/><u>Clique ID</u>: $cliqueLink</p>";
