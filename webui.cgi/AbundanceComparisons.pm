...
sub printQueryForm {
    ...
    my $html = qq{<h1>Function Comparisons</h1>
	<p>
		The <strong>Function Comparison</strong> tool displays pairwise comparisons between <em>one query</em> (meta)genome and <em>multiple reference</em> (meta)genomes, including estimates of the statistical significance of the observed differences.
	</p>};

    $html .= '<form method="post" action="main.cgi" enctype="multipart/form-data" name="mainForm">';
    $html .= returnStatusLine( "Loading ...", 1 );

    $html .= printJavaScript();
...
<h4>Function</h4>
<p>
<input type='radio' id="fn_cog" name='function' value='cog' checked /> <label for="fn_cog">COG</label>
<input type='radio' id="fn_enzyme" name='function' value='enzyme' /> <label for="fn_enzyme">Enzyme</label>
<input type='radio' id="fn_ko" name='function' value='ko' /> <label for="fn_ko">KO</label>
<input type='radio' id="fn_pfam" name='function' value='pfam' /> <label for="fn_pfam">Pfam</label>
<input type='radio' id="fn_tigr" name='function' value='tigrfam' /> <label for="fn_tigr">TIGRfam</label>
</p>};
	...
	$html .= q{
    <h4>Measurement</h4>
    <p><input type='radio' id='xcopy_gene_count' name='xcopy' value='gene_count' checked />
    <label for='xcopy_gene_count'>Gene count</label>
    <input type='radio' id="xcopy_est_copy' name='xcopy' value='est_copy' /><label for='xcopy_est_copy'>};
    $html .= WebUtil::returnInfoTipLink
    ...
    <h4>Output Type</h4>
    <p><input type='radio' id='outType_dscore' name='outType' value='dscore' checked /> <label for='outType_dscore'>Two-proportion Z-Test with pooled variance</label> (}
	. alink($url1, "Z-Test", "_blank")
	. q{)<br>
    <input type='radio' id='outType_zlor' name='outType' value='zlor' /> <label for='outType_zlor'>Z normalized Log Odds Ratio</label> (}
	. alink($url2, "Z-LOR", "_blank")
	. q{)</p>
    </div>

    <div style='width:500px; float:left;'>
    <h4>Display</h4>
    <p><input type='radio' id='rowType_allRows' name='rowType' value='allRows' checked />
    <label for='rowType_allRows'>Show all rows</label>
    <input type='radio' id='rowType_nonZeroRows' name='rowType' value='nonZeroRows' />
    <label for='rowType_nonZeroRows'>Show only rows with at least one non-zero gene count</label>
    <input type='radio' id='rowType_significantRows' name='rowType' value='significantRows' />
    <label for='rowType_significantRows'>Show only rows with significant hits</label>
    </p>

    <p>Minimum function gene count (}
	. alink($url1, "Z-Test", "_blank") . ")";
    $html .= popup_menu(
            -name    => "mincount",
    ...
    $html = reset( -id => "reset", -class => "smbutton" );
    $html .= q{
    </p>
    </div>
...
sub writePagerFiles {
    ...
    for my $funcId (@funcIds) {
        ...
        # select
        $r .= "\t";
        $r .= "<input type='checkbox' id='$idType_$funcId' name='$idType' value='$funcId' />\t";
        $r .= "\t";
