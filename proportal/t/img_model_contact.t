...
subtest 'Model building' => sub {
	...
	subtest 'valid' => sub {
# 	my $contact = IMG::Model::Contact->new();
# 	throws_ok { $contact->username } qr[Missing required arguments];
		...
		ok( $c2->set_username('Carmen Sandiego'), 'resetting username' );
		ok( $c2->has_username && 'Carmen Sandiego' eq $c2->username, 'new username is ' . $c2->username ) or diag explain $contact;
	};
...
subtest 'Models with DB info' => sub {
	...
	ok( $app->_set_user( $user_h->{100055} ), 'can add new user' );
	ok( $app->user->contact_oid == 100055, 'contact_oid is ' . $app->user->contact_oid );
