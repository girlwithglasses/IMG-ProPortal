...
subtest 'template errors' => sub {
    ...
    throws_ok { $app->render_template({ tmpl => undef, tmpl_data => 'hello mum' }) } qr[No template name specified];

    throws_ok { $app->render_template({ tmpl => '' }) } qr[No template name specified];

    $app = TemplateApp->new( config => { tmpl_dir => 'a/completely/made/up/path' } );
    throws_ok { $app->render_template({ tmpl => 'ani_home.tt' }) } qr[file error - ani_home.tt: not found];
...
subtest 'template rendering' => sub {
	...
	my $output = $tmpl_app->render_template({ tmpl => 'any_content.tt', tmpl_data => { title => 'My Cool Page', content => { fee => 'fi', fo => 'fum' } } });
