...
subtest 'directory_exists' => sub {
	...
	run_test({
		config => {},
		http_params => {},
		err => $errors->{unavailable_msg}->( err({ err => 'fs_unavailable' }) ),
	...
	TODO : {
		local $TODO = 'implement timed functions!';
...
sub run_test {
	...
	my $app = IMG::App->new( config => $args->{config}, http_params => $args->{http_params} );

#	say Dumper $app;

	my $resp = $app->run_checks( $args->{check_args} || {} );

	is_deeply( $resp, $args->{err}, $args->{msg} ) or diag explain $resp;
