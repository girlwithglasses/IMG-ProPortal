...
declare -A WORKERS=( [dev]=16 [test]=8 )
...
# starman command
STARMAN="starman --workers ${WORKERS[$CURRENT]} --error-log $ERROR_LOG $APP_HOME/bin/app.psgi"

PLACKUP="plackup -E ${ENV[$CURRENT]} -s Starman --workers=${WORKERS[$CURRENT]}"
...
#alias ssdev="start_server --dir=$HOME/webUI/proportal --port=5009 --pid-file=$HOME/proportal_dev.pid --status-file=$HOME/proportal_dev.status --log-file=$HOME/apache/logs/proportal-dev.psgi.log --daemonize -- plackup -E proportal-dev -s Starman --workers=16 bin/server-app.psgi"
#alias sstest="start_server --dir=$HOME/webUI/proportal --port=5013 --pid-file=$HOME/proportal_test.pid --status-file=$HOME/proportal_test.status --log-file=$HOME/apache/logs/proportal-test.psgi.log --daemonize -- plackup -E proportal-test -s Starman --workers=16 bin/server-app.psgi"
