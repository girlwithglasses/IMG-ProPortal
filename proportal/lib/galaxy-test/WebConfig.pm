...
#
# $Id: WebConfig-img-proportal.pm 35945 2016-08-02 00:44:50Z aireland $
#
...
sub getEnv {
    ...
    $e->{ domain_name } = "img-proportal-dev.jgi.doe.gov";
    $e->{ http } = "https://";
    $e->{ top_base_url } = $e->{ http } . $e->{ domain_name } . "/";

#    my $urlTag = "proportal"; # sub url directory e.g. img_ken
	my $urlTag = "";
    $e->{ urlTag } = $urlTag;
    my $dbTag = $e->{ dbTag };

#    $e->{ base_url } = $e->{ http }. $e->{ domain_name } . "/$urlTag";
    $e->{ base_url } = $e->{ http }. $e->{ domain_name } . "/img";
    $e->{ arch } = "x86_64-linux";
...
#    $e->{top_base_dir}   = $apacheVhostDir . $e->{domain_name} . "/htdocs/";
	$e->{top_base_dir} = $apacheVhostDir . 'webUI';
#    $e->{ base_dir } = $apacheVhostDir . $e->{ domain_name } . "/htdocs/$urlTag";
    $e->{ base_dir } = $apacheVhostDir . 'webUI/webui.htd';
    $e->{ cgi_url } = $e->{ base_url } . "/cgi-bin/$urlTag";
    ...
    $e->{enable_workspace} = 0;

    # free img site - no login
    $e->{enable_carts} = 1;
    $e->{cart_max_szie} = 1000; # no login max 1000
    $e->{enable_download} = 0;
    $e->{blast_max_genome} = 25;
...
#    $e->{ ma_bin } = $e->{ bin_dir } . "/ma";
    ...
    $e->{cog_data_file}     = $e->{ webfs_data_dir } ."functions/cogStatsPublic.txt";
    $e->{enzyme_data_file}  = $e->{ webfs_data_dir } ."functions/enzymeStatsPublic.txt";

    $e->{kegg_data_file}     = $e->{ webfs_data_dir } ."functions/keggModStatsPublic.txt";

    $e->{kog_data_file}     = $e->{ webfs_data_dir } ."functions/kogStatsPublic.txt";

    $e->{ko_data_file}     = $e->{ webfs_data_dir } ."functions/koStatsPublic.txt";

    $e->{pfam_data_file}    = $e->{ webfs_data_dir } ."functions/pfamStatsPublic.txt";
    $e->{tigrfam_data_file} = $e->{ webfs_data_dir } ."functions/tfamStatsPublic.txt";

    # domain_stats_file
    $e->{domain_stats_file} = $e->{webfs_data_dir} . "ui/prod/domain_stats_m_v400.txt";
