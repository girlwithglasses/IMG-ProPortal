...
sub urlize_params {
	my $hash = shift;
	if ( ref($hash) =~ /^hash/i ) {
		log_debug { 'hash to urlize params: ' . Dumper $hash };
		return '' unless keys %$hash;

		if ( 'Hash::MultiValue' eq ref $hash ) {
			return '?' . join "&amp;", map {
				my $hk = $_;
				join "&amp;",
				map { $hk . "=" . escape( $_ ) }
				( $hash->get_all( $hk ) || ( $hash->{$hk} ) );
			} sort keys %$hash;
		}
...
#	genes in cassette:
#	main.cgi?section=TaxonDetail&page=geneCassette&taxon_oid=2634166546

#	cassettes:
#	main.cgi?section=GeneCassette&page=occurrence&taxon_oid=2634166547
	cassette => sub {
		return {
# 			my $h = shift;
# 			return $h->{base}
