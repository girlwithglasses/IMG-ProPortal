...
sub get_menus {
	...
	my $config = shift // croak "get_menus requires a configuration hash for URL generation";
	...
	$url->{main_cgi_url} = $config->{main_cgi_url};
	$url->{pp_app} = $config->{pp_app};

	return {
		menu_bar => img_menu_bar(),
		section_nav => proportal(),
	};
...
sub img_menu_bar {
	...
	my $menu = {
		L => [
			...
			compare_genomes(), # $self->compare_genomes,
...
sub abc {
	...
	{ url => $url->{main_cgi_url} . '?section=np', label => 'ABC',
		submenu =>
		[
			{ url => $url->{main_cgi_url} . '?section=BcNpIDSearch&amp;option=np', label => 'Search BC/SM by ID' },
			{ url => $url->{main_cgi_url} . '?section=BiosyntheticStats&amp;page=stats', label => 'Biosynthetic Clusters' },
			{ url => $url->{main_cgi_url} . '?section=BcSearch&amp;page=bcSearch', label => 'Search BCs' },
			{ url => $url->{main_cgi_url} . '?section=NaturalProd&amp;page=list', label => 'Secondary Metabolites' },
			{ url => $url->{main_cgi_url} . '?section=BcSearch&amp;page=npSearches', label => 'Search SMs' },
		],
	};
}
...
return
	{ url => $url->{server} . '/mer/doc/about_index.html', label => 'Using',
		submenu =>
		[
...
sub proportal {
	...
	return {
		url => $url->{pp_app}, label => 'IMG ProPortal', submenu =>
		[	{	url => $url->{pp_app} . 'clade', label => 'by clade', },
			{	url => $url->{pp_app} . 'data_type', label => 'by data type', },
			{	url => $url->{pp_app} . 'location', label => 'by location', },
			{	url => $url->{pp_app} . 'phylogram.html', label => 'phylogram (work in progress)', },
		]
	};
